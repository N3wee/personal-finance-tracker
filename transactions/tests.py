...
from transactions.forms import BudgetForm, TransactionForm
...
class TransactionViewTest(TestCase):
    def setUp(self):
        ...
        self.admin = User.objects.create_superuser(
            username="nay_s", password="adminpass123", email="nay_s@example.com"
        )
        self.transaction = Transaction.objects.create(
            user=self.user,
            title="Test Income",
    ...
    def test_transaction_list_unauthenticated(self):
        ...
        self.assertRedirects(
            response, "/accounts/login/?next=/transactions/transactions/"
        )
    ...
    def test_edit_transaction_non_owner(self):
        ...
        response = self.client.post(
            reverse("edit_transaction", kwargs={
    ...
    def test_delete_transaction_owner(self):
        ...
        response = self.client.post(
            reverse(
                "delete_transaction", kwargs={"transaction_id": self.transaction.id}
            )
        )
        self.assertEqual(response.status_code, 302)
...
class BudgetViewTest(TestCase):
    ...
    def test_delete_budget_owner(self):
        self.client.login(username="testuser", password="testpass123")
        response = self.client.post(
            reverse("delete_budget", kwargs={"budget_id": self.budget.id})
        )
        self.assertEqual(response.status_code, 302)
...
class ViewTest(TestCase):
    ...
    def test_download_report_unauthenticated(self):
        ...
        self.assertEqual(response.status_code, 302)
        self.assertRedirects(
            response, "/accounts/login/?next=/transactions/download_report/"
        )
